...
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost/mock').catch((err) => console.warn('MongoDB initial connection error:', err));
...
// Database offline fallback middleware
app.use((err, req, res, next) => {
  if (err && (err.name === 'MongooseError' || err.name === 'MongoNetworkError' || err.message.includes('buffering timed out') || err.message.includes('topology'))) {
    console.warn('[AI Studio] Database offline — returning mock empty or safe response');
    if (req.method === 'GET') {
...
app.listen(app.get('port'), () => {
  ...
  const port = colonIndex !== -1 && colonIndex > 5 ? parseInt(BASE_URL.slice(colonIndex + 1), 10) : NaN;
...
`,
  ...
  } else if (!isNaN(port) && app.get('port') !== port) {
